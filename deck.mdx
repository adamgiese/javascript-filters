...
```
_`negate` accepts a function & returns a new function_
...
const valueAtLeast = doesValueCompare(isAboveOrEqual);
const valueAtMost  = doesValueCompare(isBelowOrEqual);
const valueEquals  = doesValueCompare(isEqual);

const matchesCuisine = valueEquals('cuisine');
